...
export const StoryPanel = ({ dropdown, navbarTheme, onLinkClick }: { dropdown: DropdownData, navbarTheme: "dark" | "light", onLinkClick: () => void }) => (
	<div className="flex h-[450px] relative overflow-hidden">
		...
		<div className="flex-1 p-8 flex flex-col justify-center relative">
			...
			<div className="relative z-10">
				...
				<div className="inline-flex items-center gap-3 mb-6 p-3 rounded-full bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-400/20">
					...
					<span className={`font-russo text-xs tracking-[0.3em] uppercase font-bold ${
				...
				<h2 className={`font-russo text-4xl font-bold mb-6 leading-tight ${
				...
				<p className={`text-lg leading-relaxed font-outfit font-medium mb-8 max-w-md ${
					navbarTheme === 'dark' ? 'text-white/90' : 'text-black/90'
				}`}>
					We are <span className="font-semibold text-blue-400">architects of the impossible</span>, crafting immersive experiences that blur the line between dreams and digital reality.
				</p>
			...
			<div className="grid grid-cols-2 gap-6 relative z-10">
				{dropdown.links.map((link, index) => (
					...
					>
						...
						<div className={`absolute -left-3 top-6 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-500 ${
							navbarTheme === 'dark' 
						...
						<div className="relative z-10">
							<div className={`font-russo text-base font-bold mb-2 ${
								navbarTheme === 'dark' ? 'text-white group-hover:text-blue-300' : 'text-black group-hover:text-blue-700'
							} transition-colors duration-300`}>
								{link.label}
							</div>
							<div className={`text-sm leading-relaxed ${
								navbarTheme === 'dark' ? 'text-white/80 group-hover:text-white/95' : 'text-black/80 group-hover:text-black/95'
							} transition-colors duration-300`}>
								{link.description}
							</div>
						</div>
						...
						<div className="absolute bottom-3 right-3 w-6 h-6 opacity-50 group-hover:opacity-100 transition-opacity duration-300">
							<div className={`absolute bottom-0 right-0 w-3 h-3 border-r-2 border-b-2 transition-all duration-300 ${
								navbarTheme === 'dark' 
		...
		<div className="w-72 relative overflow-hidden">
			...
			{/* Spline container with enhanced styling */}
