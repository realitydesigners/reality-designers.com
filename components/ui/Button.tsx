...
export default function Button({ 
	...
	const baseClasses = "relative inline-flex items-center justify-center font-russo font-bold uppercase tracking-wide transition-all duration-300 transform-gpu overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed";
	...
	const variantClasses = {
		...
		secondary: theme === "dark"
			? "bg-black text-white border border-black hover:bg-gray-900 hover:border-gray-900 hover:scale-105 hover:shadow-xl hover:shadow-black/20"
			: "bg-white text-black border border-white hover:bg-gray-100 hover:border-gray-100 hover:scale-105 hover:shadow-xl hover:shadow-white/20",
		outline: "bg-transparent text-current border border-current hover:bg-current hover:text-white hover:scale-105",
		navbar: theme === "dark" 
	...
	const ButtonContent = () => (
		<>
			...
			<div className={`absolute top-0 left-0 w-1.5 h-1.5 border-l border-t border-current opacity-0 group-hover:opacity-40 transition-all duration-300 ${
				variant === "navbar" ? "group-hover:scale-125" : "group-hover:scale-150"
			}`}></div>
			<div className={`absolute top-0 right-0 w-1.5 h-1.5 border-r border-t border-current opacity-0 group-hover:opacity-40 transition-all duration-300 ${
				variant === "navbar" ? "group-hover:scale-125" : "group-hover:scale-150"
			}`}></div>
			<div className={`absolute bottom-0 left-0 w-1.5 h-1.5 border-l border-b border-current opacity-0 group-hover:opacity-40 transition-all duration-300 ${
				variant === "navbar" ? "group-hover:scale-125" : "group-hover:scale-150"
			}`}></div>
			<div className={`absolute bottom-0 right-0 w-1.5 h-1.5 border-r border-b border-current opacity-0 group-hover:opacity-40 transition-all duration-300 ${
				variant === "navbar" ? "group-hover:scale-125" : "group-hover:scale-150"
			}`}></div>
		</>
	);
