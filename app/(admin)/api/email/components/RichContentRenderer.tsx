...
const ImageBlock: React.FC<{ image: any }> = ({ image }) => (
	<Section className="text-center mb-6">
		...
		/>
		{image.alt && (
			<Text className="text-[12px] text-gray-500 italic mt-2">{image.alt}</Text>
		)}
	</Section>
);
		...
		<div>
			{itemsToRender.map((item, index) => {
				...
				switch (item._type) {
					...
					case "image":
						return item.image ? (
							<ImageBlock key={index} image={item.image} />
						) : null;
			...
			{content.length > maxItems && (
				<Section className="text-center mt-4 p-3 bg-gray-100 rounded-lg">
					<Text className="text-gray-600 text-[14px]">
						...and {content.length - maxItems} more content blocks in the full
						post
					</Text>
				</Section>
			)}
